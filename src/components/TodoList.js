...
export default function TodoList({ todo }) {
    ...
    ${todo.isCompleted ? "bg-teal-700" : "bg-gray-500"}`}
        >
            {isEdit ? (
                <input
            ...
            <button
                onClick={onComplete}
                className="bg-fuchsia-700 mx-1 p-1 px-2 text-sm rounded"
            >
                {!todo.isCompleted ? "Done" : "Not Done"}
            </button>
            <button
                onClick={onEdit}
                className="bg-blue-700 mx-1 p-1 px-2 text-sm rounded"
            >
                Edit
            </button>
            <button
                onClick={onDelete}
                className="bg-red-700 mx-1 p-1 px-2 text-sm rounded"
            >
                Delete
            </button>
        </div>
